...
**СДЕЛАНО** - Использовать Serilog
- Вывод событий в консоль: начало и завершение импорта, пропущенные файлы, итоговый отчёт.
**СДЕЛАНО** - При завершении отображается сводка:
- общее число найденных файлов,
**СДЕЛАНО** - число успешно сохранённых,
  - число пропущенных (с разбивкой по причинам: «битый», «нет метаданных», «неподдерживаемый формат»).
**СДЕЛАНО** - Уровни: `Info` (ход выполнения), `Warn` (пропуски), `Error` (критические ошибки).
**СДЕЛАНО** - Цветной вывод в терминале; автоматически отключается при перенаправлении (`|`, `>`).
