...
- Найти **все** аудиофайлы, независимо от расширения.
- Определить формат файла по его содержимому (сигнатура/заголовок), а не по имени.

## 3. Обработка файла
- Для каждого корректного аудиофайла извлечь метаданные (точный набор — уточняется позже).
- Если файл:
    - повреждён,
...
## 4. Сохранение в БД
- Записать данные о треке в базу данных.
...
- Поддерживать **режим сухого прогона (dry-run)**:
    - показать, какие действия *были бы* выполнены,
    - не вносить изменений в файловую систему и БД.
